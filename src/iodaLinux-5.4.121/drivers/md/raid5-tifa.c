...
ssize_t tifa_show_rdlat(struct mddev *mddev, char *page)
{
	...
	printk("IODA, rdlat_idx=%llu\n", nr);
	for (i = 0; i < nr; i++) {
		printk("IODA, i:%llu,%llu\n", i, rdlat_array[i]);
	}
...
#define TSZ 5000000
	...
	if (new == 0) {
		if (rdlat_array) {
			printk("IODA, %s,rdlat_array already initialized ..\n",
			       __func__);
			return 0;
		} else {
			printk("IODA, %s,initialize rdlat array\n", __func__);
		...
		if (rdlat_array) {
			...
			printk("IODA, rdlat_array=NULL not initialized yet !!\n");
		}
		printk("IODA, %s,rdlat_idx reset to %d\n", __func__,
		       atomic_read(&rdlat_idx));
	} else if (new >= 1000000) {
		printk("IODA, re-allocate rdlat_array to be have %ld entries\n",
		       new);
		if (rdlat_array) {
			vfree(rdlat_array);
		}
